    ...
    for index in line.indices {
        if let digit = Int(String(line[index])) {
            ...
            for (key, value) in map {
                if line.suffix(from: index).hasPrefix(key) {
                    return value
                }
            }
        }
...
func lastDigit(line: String) -> Int {
    for index in line.indices.reversed() {
        if let digit = Int(String(line[index])) {
            ...
            print("testing", line.prefix(upTo: index))
            for (key, value) in map {
                if line.prefix(upTo: line.index(after: index)).hasSuffix(key) {
                    return value
                }
            }
        }
...
public let day1 = Solution(
    part1: { input in
        return input.split(whereSeparator: \.isNewline)
            .map { line in
                let integers = line.compactMap { Int(String($0)) }
                return integers.first! * 10 + integers.last!
            }
            .sum
    },
    part2: { input in
        return input.split(whereSeparator: \.isNewline)
            .map(String.init)
            .map { line in
                return firstDigit(line: line) * 10 + lastDigit(line: line)
            }
            .sum
        //54627 too low
    },
