...
private extension Grid {
    func neighbours(to point: Grid.Point) -> Set<Grid.Point> {
        data.filter { other in
            abs(other.x - point.x) <= 1 &&
            abs(other.y - point.y) <= 1 &&
            other.distance(to: point) > 0
        }
    }
}

func numbersWithPositions(lines: [String.SubSequence]) -> [(val: Int, points: [Grid<Character>.Point])] {
    return lines
        .enumerated()
        .flatMap { y, line in
            line.split(whereSeparator: { !$0.isNumber })
                .map { intString in
                    let int = Int(String(intString))!
                    return (
                        val: int,
                        points: intString.indices
                            .map { line.distance(from: line.startIndex, to: $0) }
                            .map { x in
                                Grid.Point(x: x, y: y, val: Character(" "))
                            }
                    )
                }
        }
...
public let day3 = Solution(
    part1: { input in
        ...
        let grid = Grid(lines: lines.map { $0.map { $0 }})
        let numbers = numbersWithPositions(lines: lines)
        let filtered = numbers.filter { _, points in
            points.contains { point in
                grid.neighbours(to: point)
                    .contains { other in
                        !other.val.isNumber && other.val != "."
                    }
            }
        }
        return filtered.map(\.val).sum
    },
    part2: { input in
        let lines = input.split(whereSeparator: \.isNewline)
        let grid = Grid(lines: lines.map { $0.map { $0 }})
        let numbers = numbersWithPositions(lines: lines)
        let gears = grid.data.filter { $0.val == "*" }
        let geared = gears.map { gear in
            Set(
                grid.neighbours(to: gear)
                    .compactMap { neighbour in
                        numbers
                            .first(where: { $0.points.contains(where: { $0.distance(to: neighbour) == 0 })})?
                            .val
                    }
                )
        }
            .filter { $0.count == 2 }
        return geared.map { $0.reduce(1, *) }.sum
    },
    testResult: (4361, 467835),
    testInput: """
467..114..
...*......
..35..633.
