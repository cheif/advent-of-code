...
func games(input: String) -> [Int: [[Cube: Int]]] {
    var games: [Int: [[Cube: Int]]] = [:]
    let lines = input.split(whereSeparator: \.isNewline)
    for line in lines {
        let splitted = line.split(separator: ": ")
        assert(splitted.count == 2)
        let id = Int(splitted[0].split(separator: " ")[1])!
        let sets = splitted[1].split(separator: ";")
            .map { set -> [Cube: Int] in
                let list = set.split(separator: ", ").map { spec in
                    let parts = spec.split(separator: " ")
                    assert(parts.count == 2)
                    return (Cube(rawValue: String(parts[1]))!, Int(String(parts[0]))!)
                }
                return Dictionary(list, uniquingKeysWith: { lhs, _ in lhs })
            }
        games[id] = sets
    }
    return games
...
public let day2 = Solution(
    part1: { input in
        let games = games(input: input)
        let valid = games.filter { (key, sets) in
            sets.allSatisfy { set in
                (set[.red] ?? 0) <= 12 && (set[.green] ?? 0) <= 13 && (set[.blue] ?? 0) <= 14
            }
        }
        return valid.map(\.key).sum
    },
    part2: { input in
        let games = games(input: input)
        let minimumSet = games.mapValues { sets in
            sets.reduce(into: [Cube: Int]()) { partialResult, set in
                for (cube, count) in set {
                    partialResult[cube] = max(partialResult[cube] ?? 0, count)
                }
            }
        }
        let powers = minimumSet.mapValues { set in
            set[.red]! * set[.green]! * set[.blue]!
        }
        return powers.map(\.value).sum
    },
    testResult: (8, 2286),
    testInput: """
